...
//if((_G==undefined)||(_G==null))
  _G.fio=window.admin_INIT.fio;
  _G.prefix="../";
  _G.cL=window.admin_INIT.collsList;
  _G.wUser=window.admin_INIT.wUser;
  _G.job=window.admin_INIT.jobs;
  ...
  _G.selC = document.getElementById("selC");
  ...
  _G.impSP=document.getElementById("conV");
  _G.helP=window.admin_INIT.helP;
  ...
  document.getElementById("selC").style.cursor = "pointer";
  document.getElementById("impB").style.cursor = "pointer";
  document.getElementById("tstB").style.cursor = "pointer";
  document.getElementById("expB").style.cursor = "pointer";
  document.getElementById("conV").style.cursor = "pointer";
  document.getElementById("userP").style.cursor = "pointer"; // кнопка "Пользователи"
  document.getElementById("savUP").style.cursor = "pointer";
  let tr, oT=document.getElementById("collL");
  for(tr in oT.rows){
    oT.rows[tr].onclick = getDocs;
  }
  document.getElementById("expB").addEventListener("click",expBase);
  document.getElementById("impB").addEventListener("click",impBase);
  document.getElementById("tstB").addEventListener("click",tstBase);
  document.getElementById("conV").addEventListener("click",impRules);
  ...
  _G.extTH.addEventListener("input",impVis);
  ...
  document.getElementById("userP").addEventListener("click", userPanel); // кнопка "Пользователи"
  document.getElementById("savUP").addEventListener("click", saveUP);
  _G.timerI=setInterval(timer, 60000);

  _G.divTun.style.display="none";
  setInterval(_G.test, 500);
  _G.dTime=window.admin_INIT.dTime;
  _G.tplName=window.admin_INIT.tplName;
  _G.autoS=window.admin_INIT.autoS;
  _G.saveNN=window.admin_INIT.saveNN;
  ...
  if (window.admin_INIT.rulesList.length===0) {
    _G.mongo=false;
    _G.dTime=5000;
...
//  _G.extTH.value=docH;
  ...
  let dimp=document.getElementById("impC");
  if(dimp!=null){
    dimp.style.left=_G.divMnu.offsetLeft + _G.divMnu.clientWidth + 2 + "px";
    dimp.style.width=docElem.scrollWidth - _G.divRules.offsetLeft - 20 + "px";
    dimp.style.top=_G.divMnu.offsetTop+_G.divMnu.scrollHeight-dimp.scrollHeight+"px";
    dimp.style.display="none";
  }
  dimp=document.getElementById("tstD");
  if(dimp!=null){
    dimp.style.left=_G.divMnu.offsetLeft + _G.divMnu.clientWidth + 2 + "px";
    dimp.style.width=docElem.scrollWidth - _G.divRules.offsetLeft - 20 + "px";
    dimp.style.top=_G.divMnu.offsetTop+_G.divMnu.scrollHeight-dimp.scrollHeight+"px";
...
function shOpenC(event){
  _G.testing = document.getElementById("collOp");
  _G.start="";
  if(_G.cL.length>10)
    window.onkeypress=wKeyPress;
  showOpenR(event.target);
}
function wKeyPress(){
  if(document.getElementById("collOp").style.display!=="none"){
    if(window.event.key=="Enter"){
      getDocs(document.getElementById("collL").rows[1].cells[0].innerText)
      return;
    }
    _G.start+=window.event.key;
    let i,j,l=_G.cL.length;
    let t=document.getElementById("collL");
    let h="";
    for(i=0;i<l;i++)
      if(_G.cL[i].substring(0,l)>=_G.start)
        break;
    if(_G.cL.length-i<10)
      i=_G.cL.length-10;
    j=0;
    if(i>0){
      h+="<tr><td><span>...</span>";
    ...
    for(i=0;i<t.rows.length;i++){
      if(t.rows[i].cells[0].innerText.indexOf("..."<0))
        t.rows[i].onclick = getDocs;
    }
  }
}
function vJob(v){
...
function stop(e){
  if(_G.rulesChng&&(_G.autoS===2))
    saveRules();
  //openRules(e,_G.prefix+"goOut.jsp");
  openRules(e, "/logout");
}
...
function viewSwitch(w){
  if(document.getElementById("impC")!==null)
    document.getElementById("impC").style.display="none";
  if(document.getElementById("tstD")!==null)
    document.getElementById("tstD").style.display="none";
  if(document.getElementById("savUP")!==null)
    document.getElementById("savUP").style.visibility="hidden";
  _G.txtCod.removeEventListener("change",passChange);
  switch (w) {
    case 1:
      _G.divRes.style.visibility="hidden";
    ...
    case 3:
      ...
      _G.divTun.style.display="none";
      _G.ifr.style.display="none";
      if((_G.txtCod.pass!=undefined)&&(_G.txtCod.pass!="")){
        if(document.getElementById("savUP")!==null)
          document.getElementById("savUP").style.visibility="visible";
        _G.txtCod.addEventListener("change",passChange);
      }
        break;
    case 4:
    ...
    case 6:
      ...
      document.getElementById("impC").style.display="block";
      _G.divAny.style.top=_G.divExt.offsetTop+"px";
      _G.divAny.style.left=_G.divExt.offsetLeft-2+"px";
      _G.divAny.style.width=document.getElementById("impC").scrollWidth+"px";
      _G.divAny.style.height=_G.divMnu.scrollHeight-document.getElementById("impC").scrollHeight+"px";
      _G.divExt.style.visibility="hidden";
      _G.divRules.style.visibility="hidden";
      _G.divAny.style.visibility="visible";
      _G.txtCod.style.display="none";
      ...
      break;
    case 7:
      document.getElementById("tstD").style.display="block";
      _G.divAny.style.top=_G.divExt.offsetTop+"px";
      _G.divAny.style.left=_G.divExt.offsetLeft-2+"px";
      _G.divAny.style.width=document.getElementById("tstD").scrollWidth+"px";
      _G.divAny.style.height=_G.divMnu.scrollHeight-document.getElementById("tstD").scrollHeight+"px";
      _G.divExt.style.visibility="hidden";
      _G.divRules.style.visibility="hidden";
      _G.divAny.style.visibility="visible";
      _G.txtCod.style.display="none";
...
function runT(){
  ...
  script.src=_G.prefix+"users/"+_G.wUser+"translator.txt?v="+(""+Math.random()).substring(2,6);
...
/*      if((re.length===0)||"Error:"===re.substring(0,6)){
  ...
  xmlhttp.open("GET","/file?filename="+_G.wUser+"translator.txt",true);
  xmlhttp.responseType = 'text';
  xmlhttp.send();
  timeClear();
}
function docToMenu(re){
  let tbl=_G.openT;
  let i=0,s;
  ...
  while(re!==""){
    ...
    if(i<tbl.rows.length){
      tbl.rows[i].cells[0].innertHTML="&nbsp;<span>"+s+"&nbsp;</span>";
    }else{
      let r=tbl.insertRow(i);
  ...
  if(tbl.rows.length>0){
    _G.opnM.classList.remove("grSp");
    if(document.getElementById("cDel")!=null)
      document.getElementById("cDel").classList.remove("grSp");
  }else if((_G.opnM.classList=="")||(_G.opnM.classList.indexOf("grSp")<0)){
    _G.opnM.classList.add("grSp");
    if(document.getElementById("cDel")!=null)
      document.getElementById("cDel").classList.add("grSp");
  }
}
...
//  let cll = "";
  ...
  var nam;
  if(typeof(nm)=="object")
    nam=this.cells[0].innerText.trim();
  else
    nam=nm;
  if(nam==="...")
    return;
  let ind=_G.fio.indexOf(nam)+nam.length;
  let fio=_G.fio.substr(ind,50);
  ind=fio.indexOf(" ");
  if(fio.indexOf("#")<ind)
    ind=fio.indexOf("#");
  fio=fio.substr(1,ind-1);
  ...
  xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState===4 && xmlhttp.status===200){
      let re = xmlhttp.responseText.trim();
      if("Error:"===re.substring(0,6)){ //(re.length===0)||
        alert("Не удалось открыть систему правил. "+re);
        return;
      }
      if("<"===re.substring(0,1)){
        if((re.indexOf("password")>0)||(url==="/logout"/*_G.prefix+"goOut.jsp"*/))
          //document.location.href=_G.prefix+"index.jsp";
...
//        stop();
  ...
  // !!!
  // url = "../work/getDocs.jsp?name=" + nam;
  let url = "/work/getDocs?name" + nam;
  document.getElementById("asUser").innerText=" "+nam+" ";
  rClear();
...
function delColl(){
  ...
  if(!_G.mongo)
    return;
  let nam=document.getElementById("asUser").innerText.trim();
  if(nam===_G.wUser)
    return;
  if(!confirm("Вы действительно хотите удалить коллекцию '"+nam+"'?"))
    return;
  let xmlhttp;
  if (window.XMLHttpRequest) {
    xmlhttp=new XMLHttpRequest();
  ...
  xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState===4 && xmlhttp.status===200){
      let re = xmlhttp.responseText.trim();
      if("Error:"===re.substring(0,6)){ //(re.length===0)||
        alert("Не удалось удалить коллекцию. "+re);
        return;
      }
      if("<"===re.substring(0,1)){
        if((re.indexOf("password")>0)||(url==="/logout"/*_G.prefix+"goOut.jsp"*/))
         ...
         alert("Не удалось удалить коллекцию. Попробуйте выйти и войти заново.");
//        stop();
        return;
      }
    }
  }
  // dropColl.jsp нет, есть только dropColls. игнорируем
  url = "dropColl.jsp?name=" + nam;
...
function openRules(e,url){
  viewSwitch(1);
  if((url!=="/logout"/*_G.prefix+"goOut.jsp"*/)&&(!_G.mongo||(this.className === "coll")))
    return;
  let cll = "";
  ...
  if(url!=="/logout"/*_G.prefix+"goOut.jsp"*/){
    let nam=this.cells[0].innerText.trim();
    //url = _G.prefix+"getRules.jsp?coll="+document.getElementById("asUser").innerText.trim()+"&name=" + (nam===""?"noName":nam);
    url = "/rules/getRules?name="+document.getElementById("asUser").innerText.trim()+"&coll=" + (nam===""?"noName":nam);
...
function saveRules(){
  if(!_G.mongo)
    return;
  if(_G.wUser!==document.getElementById("asUser").innerText.trim()){
    alert("Нельзя сохранять систему правил в чужой коллекции.");
    return;
  }
  let rN=_G.rnamE;
  if(rN.value==='')
...
function rulesDel(){
  if(!_G.mongo||((_G.rdeL!=0)&&(_G.rdeL.classList.toString().indexOf("grSp")>=0)))
    return;
  let rN=_G.rnamE.value.trim();
  if(rN==='')
    rN="noName";
...
//      _G.extTH.value=resp;
        if((what===5)||(what===6)){//Подключение скриптов к табличному аниматору сканера
          document.getElementById("tHist").style.display="block";
          for(let i=0;i<document.head.children.length;i++){ // проверяем не подключен ли скрипт SAnimator.js
            var chld=document.head.children[i];
            if((chld.toString()==="[object HTMLScriptElement]")&&((ind=chld.src.indexOf("SAnimator"))>0))
              chld.parentNode.removeChild(chld); // если скрипт уже подключен, отвязываем его
            if((chld.toString()==="[object HTMLScriptElement]")&&((ind=chld.src.indexOf("vis-net"))>0))
              chld.parentNode.removeChild(chld); // если скрипт уже подключен, отвязываем его
          }
          var script = document.createElement('script');
          script.type = 'text/javascript';
          script.src=_G.prefix+"js/SAnimator.js";
          document.head.appendChild(script); // покдлючаем скрипт
          if(what===6){
            var libVisJS = document.createElement('script');
            libVisJS.type = 'text/javascript';
            libVisJS.src = _G.prefix+'js/vis-network.min.js';
            libVisJS.defer = true;
            libVisJS.onload = function() {
              showGraph();
//              textGraph();
            }
            document.head.appendChild(libVisJS);
          }
        }
...
/*
          ...
          if(f){
            var scriptScanner = document.createElement('script');
            scriptScanner.type = 'text/javascript';
            scriptScanner.src=_G.prefix+"js/Scanner.js";
            document.head.appendChild(scriptScanner);
...
function impRules(){
  let ext=_G.extTH;
  let ind=ext.value.indexOf("видимых");
  let txt=ext.value.substr(ind);
  ind=txt.indexOf("\n");
  txt=txt.substr(ind+1).replaceAll(":\t"," ");
  _G.tR=txt.split("\n");
  ind=ext.value.indexOf("/*Синтакс");
  txt=ext.value.substring(0,ind);
  ind=txt.indexOf("е:*/");
  ext.value=txt.substr(ind+5);
  _G.analyze();
  let newRow=_G.tabR.insertRow();
  newRow.onclick = ruleClick;
  newRow.addEventListener("contextmenu",mouseButUp);
...
function expBase(){
  ...
  xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState===4 && xmlhttp.status===200){
      ...
      if(re.charAt(0)==="<"){
        ...
        alert("Не удалось экспортировать базу данных в текстовый файл: " + xmlhttp.responseText.substring(7));
      else 
        alert("Ok "+re);
    }
  }
  let url;
  // !!!
  // url = _G.prefix+"main/expBase.jsp";
  url = "/admin/expBase";
...
function toTable(re){
  let r="<table id='collsImp' style='cursor:pointer' onclick='invT();'><tr><td>Пользователь<td>Имя коллекции<td colspan=8>Имена систем правил";
  let ix=re.indexOf(":");
  let nd;
  while(ix>0){
    nd=re.indexOf(":;");
    if((nd<0)||(nd>ix)){
      r+="<tr><td>"+re.substr(0,ix);
      re=re.substr(ix+1);
      ind=re.indexOf(":");
      r+="<td>"+re.substr(0,ind);
      re=re.substr(ind+1);
      ind=re.indexOf(";");
      nd=re.indexOf(",");
      while((nd>=0)&&(nd<ind)){
  ...
  xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState===4 && xmlhttp.status===200){
      ...
      if(re.charAt(0)==="<"){
        ...
        alert("Не удалось импортировать текстовый файл в базу данных: " + xmlhttp.responseText.substring(7));
      else {
        viewSwitch(6);
//        let cp="<div></div>";
        _G.divAny.innerHTML=toTable(re);
//        document.getElementById("impB").classList.add("grSp");
//        _G.divAny.classList.remove("div");
//        alert(_G.divAny.classList.toString())
//        let t=document.getElementById("collsImp");
//        t.rows(0).cells(0).onclick=collsImpSort;
      }
    }
  }
  let url;
  // !!!
  //url = _G.prefix+"main/getContent.jsp";
...
function incA(){
  ...
  for(let i=1;i<t.rows.length;i++){
    let r=t.rows[i];
    for(let j=1;j<r.cells.length;j++)
      r.cells[j].classList.remove("no");
  }
}
function decA(){
  let t=document.getElementById("collsImp");
  for(let i=1;i<t.rows.length;i++){
    let r=t.rows[i];
    for(let j=1;j<r.cells.length;j++)
      if(!isNaN(r.cells[1].innerText.trim()))
        r.cells[j].classList.add("no");
  }
}
function invT(){
  let c=event.target;
  ...
  if((i===0)||(i===undefined))
    return;
  if(c.classList.toString().indexOf("no")<0){
    c.classList.add("no");
    if(i===1){
      for(;i<r.cells.length;i++){
        if(r.cells[i].classList.toString().indexOf("no")<0)
          r.cells[i].classList.add("no");
      }
    }else{
      for(i=2;i<r.cells.length;i++){
        if(r.cells[i].classList.toString().indexOf("no")<0)
          return;
      }
      if(r.cells[1].classList.toString().indexOf("no")<0)
        r.cells[1].classList.add("no");
    }
  }else{
    ...
    if(i===1){
      for(;i<r.cells.length;i++){
        if(r.cells[i].classList.toString().indexOf("no")>=0)
          r.cells[i].classList.remove("no");
      }
    }else{
      if(r.cells[1].classList.toString().indexOf("no")>=0)
        r.cells[1].classList.remove("no");
      for(i=2;i<r.cells.length;i++){
        if(r.cells[i].classList.toString().indexOf("no")>=0)
          return;
      }
...
function collsImpSort(c){
  if(c>1)
    return;
  let t=document.getElementById("collsImp");
  let i,j,n,k=t.rows.length;
  for(i=1;i<k;i++){
    n=i;
    let x=t.rows[i].cells[c].innerText;
    for(j=i+1;j<k;j++){
      if(x>t.rows[j].cells[c].innerText){
        n=j;
        x=t.rows[j].cells[c].innerText;
      }
    }
    if(n!==i){
      let nr=t.insertRow(i);
      nr.innerHTML=t.rows[n+1].innerHTML;
      t.deleteRow(n+1);
    }
  }
}
function execImp(){
  let iPar=0;
  if(document.getElementById("imp1").checked)
    iPar=1;
  else if(document.getElementById("imp2").checked)
    iPar=2;
  else if(document.getElementById("imp3").checked)
    iPar=3;
  else if(document.getElementById("imp4").checked){
    if(document.getElementById("preF").value.trim()===""){
      alert("Не заполнено поле префикса");
      return;
    }
    iPar=4;
  }
  ...
  let list="",i,j,k,f,r,fNo=false;
  for(i=1;i<rows;i++){
    if(t.rows[i].cells[1].classList.toString().indexOf("no")<0){
      r=t.rows[i];
      k=r.cells.length;
      f=false;
      for(j=2;j<k;j++)
        if(r.cells[j].classList.toString().indexOf("no")>=0){
          f=true;
          break;
        }
      if(f){
        fNo=true;
        list+=r.cells[1].innerText+":";
        f="";
        for(j=2;j<k;j++)
          if(r.cells[j].classList.toString().indexOf("no")<0){
            list+=f+r.cells[j].innerText;
            f=","
          }
        list+=";";
      }else
  ...
  if(list.trim()==""){
    alert("Список пуст, ничего не сделано.");
    return;
  }
  list=iPar+":"+document.getElementById("preF").value.trim()+";"+(fNo?list:"!");
//  alert(list);
//  return;
  let xmlhttp;
  if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
  ...
  xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState===4 && xmlhttp.status===200){
      ...
      if(re.charAt(0)==="<"){
        ...
        alert("Не удалось выполнить загрузку всех коллекций: " + xmlhttp.responseText.substring(7));
      else{
        /*document.getElementById("extT").value= re;
        alert(re.length);
        */
        let rx=re.split(" ");
        let rr="Обработано коллекций: "+rx[0]+"\nДобавлено систем правил: "+rx[1]+"\nИзменено систем правил: "+rx[2]+"\nНе обработано систем правил (превышение макс. к-ва): "+rx[3];
        alert(rr);
  ...
  // !!!
  //let url = _G.prefix+"main/impBase.jsp";
  let url = "/admin/impBase";
  xmlhttp.open("POST",url,true);
  xmlhttp.setRequestHeader("Content-Type", "application/json; charset=utf-8");
  xmlhttp.send(list);
  timeClear();
...
//      _G.txtCod.value=re;
  ...
  // !!!
  // let url = _G.prefix+"main/listBase.jsp";
  let url = "/admin/listBase";
  xmlhttp.open("GET",url,true);
  xmlhttp.responseType = 'text';
  xmlhttp.send();
  timeClear();
...
//  return;
  ...
  xmlhttp.onreadystatechange=function(){
    if (xmlhttp.readyState===4 && xmlhttp.status===200){
      ...
      if(re.charAt(0)==="<"){
        ...
        */
        let rx=re.split("|");
        let rr="Удалено коллекций: "+rx[0]+"\nудалено систем правил: "+rx[1];
        if(rx[2].trim()!="")
          rr+="\n"+rx[2];
        alert(rr);
      }
  ...
  // !!!
  // let url = _G.prefix+"main/dropColls.jsp?names="+list;
  let url = "/admin/dropColls?names="+list;
  xmlhttp.open("POST",url,true);
  xmlhttp.setRequestHeader("Content-Type", "application/json; charset=utf-8");
  xmlhttp.send();
  timeClear();
...
function userPanel(){
//    alert("It's alert of user panel button!");
  let xmlhttp;
  if (window.XMLHttpRequest) {
    xmlhttp=new XMLHttpRequest();
  } else {
...
/*      if((re.length===0)||"Error:"===re.substring(0,6)){
  ...
  // !!!
  //let url = _G.prefix+"main/getPlist.jsp?file=plist.txt";
  let url = "/admin/getPlist?file=plist.txt";
  xmlhttp.open("GET",url,true);
  xmlhttp.responseType = 'text';
  xmlhttp.send();
  timeClear();
...
function impVis(){
  if(_G.extTH.value.indexOf(":*/")>0)
    _G.impSP.classList.remove("grSp");
  else
    _G.impSP.classList.add("grSp");
}
function saveUP(){
  if(document.getElementById("savUP").classList.toString().indexOf("grSp")>=0)
    return;
  var iold=_G.txtCod.pass.indexOf("\n");
  var inew=_G.txtCod.value.indexOf("\n");
  _G.txtCod.value=_G.txtCod.pass.substring(0,iold)+_G.txtCod.value.substring(inew);
  let xmlhttp;
  if (window.XMLHttpRequest) {
    xmlhttp=new XMLHttpRequest();
  } else {
...
/*      if((re.length===0)||"Error:"===re.substring(0,6)){
      ...
      if(re!=="")
        ...
        if(document.getElementById("savUP")!==null)
          document.getElementById("savUP").classList.add("grSp");
      }
  }
  // !!!
  // let url = _G.prefix+"main/savePlist.jsp";
  let url = "/admin/savePlist";
  xmlhttp.open("POST",url,true);
  xmlhttp.responseType = 'text';
  xmlhttp.send(_G.txtCod.value);
  timeClear();
}
function passChange(){
  if(document.getElementById("savUP")!==null)
    document.getElementById("savUP").classList.remove("grSp");
}